<template>
	<view class="song-list-comp">
		<view class="tit-bar">
			{{title}}
			<navigator url="#" class="more fr" :url="link">歌单广场</navigator>
		</view>
		<view class="clearfix">
			<view class="item" v-for="(item,index) in list " :key="item.id">
				<image class="img" :src="item.picUrl" alt="" srcset="">
					<view class="desc">
						{{item.name}}
					</view>
					<view class="count" v-if="item.playCount>1000">
						{{(item.playCount / 10000).toFixed(1)}}w
					</view>
